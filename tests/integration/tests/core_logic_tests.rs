...
fn test_famp_baseline_freeze_authority() {
    ...
    assert!(report.passed, "Baseline freeze authority test failed: {:?}", report.error);
}
...
fn run_baseline_freeze_authority_test() -> TestResultReport {
    ...
    assertions += 1;
    if !is_authorized {
        return TestResultReport::failure(test_name, "Issuer not recognized as authorized".to_string());
    }
    ...
    assertions += 1;
    if !unauthorized {
        return TestResultReport::failure(test_name, "Non-issuer should not be authorized".to_string());
    }
    ...
    if !issuer_bypasses_gating {
        return TestResultReport::failure(test_name, "Issuer should bypass gating program".to_string());
    }
...
fn test_interface_optional_method_support() {
    ...
    assert!(report.passed, "Interface optional methods test failed: {:?}", report.error);
}
...
fn run_interface_optional_methods_test() -> TestResultReport {
    ...
    if allow_list.can_freeze_permissionless(&Pubkey::new_unique()) != GateResponse::NotSupported {
        return TestResultReport::failure(test_name, "Allow list should NOT support freeze".to_string());
    }
    ...
    if block_list.can_thaw_permissionless(&Pubkey::new_unique()) != GateResponse::NotSupported {
        return TestResultReport::failure(test_name, "Block list should NOT support thaw".to_string());
    }
    
    assertions += 1;
    if block_list.can_freeze_permissionless(&Pubkey::new_unique()) != GateResponse::Success {
        return TestResultReport::failure(test_name, "Block list should support freeze".to_string());
    }
...
fn test_permission_deescalation_security() {
    ...
    assert!(report.passed, "Permission de-escalation test failed: {:?}", report.error);
}
...
fn run_permission_deescalation_test() -> TestResultReport {
    ...
    if accounts_to_gating_program[0].is_signer {
        return TestResultReport::failure(
            test_name, 
            "User account must NOT be signer in gating program (de-escalation required)".to_string()
        );
    }
    ...
    assertions += 1;
    let all_deescalated = accounts_to_gating_program.iter()
...
fn test_gating_program_limited_power() {
    ...
    assert!(report.passed, "Gating program limitation test failed: {:?}", report.error);
}
...
fn run_gating_program_limitation_test() -> TestResultReport {
    ...
    if decision_allow != GatingDecision::Allow {
        return TestResultReport::failure(test_name, "Gating program should be able to allow".to_string());
    }
    ...
    if decision_deny != GatingDecision::Deny {
        return TestResultReport::failure(test_name, "Gating program should be able to deny".to_string());
    }
    ...
    if !gating_can_only_decide {
        return TestResultReport::failure(test_name, "Gating program should only decide".to_string());
    }
    
    assertions += 1;
    if !gating_cannot_execute {
        return TestResultReport::failure(test_name, "Gating program should not execute actions".to_string());
    }
    ...
    if !famp_respects_decision {
        return TestResultReport::failure(test_name, "FAMP should respect gating decision".to_string());
    }
...
fn test_issuer_control_with_third_party() {
    ...
    assert!(report.passed, "Issuer control test failed: {:?}", report.error);
}
...
fn run_issuer_control_test() -> TestResultReport {
    ...
    if !can_bypass_for_freeze {
        return TestResultReport::failure(test_name, "Issuer should bypass gating for permissioned freeze".to_string());
    }
    ...
    if !can_bypass_for_thaw {
        return TestResultReport::failure(test_name, "Issuer should bypass gating for permissioned thaw".to_string());
    }
    ...
    if config.gating_program != new_gating {
        return TestResultReport::failure(test_name, "Issuer should be able to change gating program".to_string());
    }
    ...
    if config.enable_permissionless_thaw {
        return TestResultReport::failure(test_name, "Issuer should be able to disable permissionless ops".to_string());
    }
    ...
    if !gating_cannot_change_authority {
        return TestResultReport::failure(test_name, "Gating program should NOT be able to change authority".to_string());
    }
    ...
    if !gating_cannot_block_issuer {
        return TestResultReport::failure(test_name, "Gating program cannot block issuer authority".to_string());
    }
...
fn test_account_permission_deescalation_validation() {
    ...
    assert!(report.passed, "Account permission de-escalation test failed: {:?}", report.error);
}
...
fn run_account_permission_deescalation_test() -> TestResultReport {
    ...
    if !famp_receives_signature {
        return TestResultReport::failure(test_name, "FAMP should receive user signature".to_string());
    }
    ...
    if !famp_can_write {
        return TestResultReport::failure(test_name, "FAMP should have write access to token account".to_string());
    }
    ...
    let can_only_read = accounts_to_gating.iter()
        .all(|acc| !acc.is_signer && !acc.is_writable);
    assertions += 1;
    if !can_only_read {
        return TestResultReport::failure(
...
fn test_gating_decision_vs_execution_separation() {
    ...
    assert!(report.passed, "Decision/execution separation test failed: {:?}", report.error);
}
...
fn run_decision_execution_separation_test() -> TestResultReport {
    ...
    if tx1.gating_decision == GatingDecision::Allow && !tx1.famp_executed {
        return TestResultReport::failure(test_name, "FAMP should execute when gating allows".to_string());
    }
    ...
    if tx2.gating_decision == GatingDecision::Deny && tx2.famp_executed {
        return TestResultReport::failure(test_name, "FAMP should NOT execute when gating denies".to_string());
    }
    ...
    if !only_famp_has_authority {
        return TestResultReport::failure(test_name, "Only FAMP should have freeze authority".to_string());
    }
...
fn test_authority_override_capability() {
    ...
    assert!(report.passed, "Authority override test failed: {:?}", report.error);
}
