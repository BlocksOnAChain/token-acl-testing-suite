...
fn test_kyc_allowlist_with_expiration() {
    ...
    assert!(report.passed, "KYC expiration test failed: {:?}", report.error);
}
...
fn test_sanctions_precedence() {
    ...
    assert!(report.passed, "Sanctions precedence test failed: {:?}", report.error);
}
...
fn test_geo_blocking() {
    ...
    assert!(report.passed, "Geo-blocking test failed: {:?}", report.error);
}
...
fn run_geo_blocking_test() -> TestResultReport {
    ...
    if security_token_gate.is_allowed(&Jurisdiction::Other) {
        return TestResultReport::failure(test_name, "Non-allowed jurisdiction granted access".to_string());
    }
...
fn test_freeze_thaw_with_revocation() {
    ...
    assert!(report.passed, "Freeze/revocation test failed: {:?}", report.error);
}
...
fn test_multistep_rwa_workflow() {
    ...
    assert!(report.passed, "Multi-step workflow test failed: {:?}", report.error);
}
...
fn run_multistep_workflow_test() -> TestResultReport {
    ...
    impl InvestorOnboarding {
        ...
        fn progress(&self) -> f32 {
            ...
            if self.kyc_complete { steps_complete += 1; }
            if self.accreditation_verified { steps_complete += 1; }
            if self.jurisdiction_allowed { steps_complete += 1; }
            if self.not_sanctioned { steps_complete += 1; }
            if self.account_created { steps_complete += 1; }
            if self.account_thawed { steps_complete += 1; }
    ...
    if successful_investor.progress() != 100.0 {
        return TestResultReport::failure(
            test_name,
            format!("Progress should be 100%, got {}", successful_investor.progress())
        );
    }
...
fn generate_advanced_scenarios_report() {
    ...
    std::fs::write("../../test-results/ADVANCED_SCENARIOS_RESULTS.md", &report).ok();
