...
const SANCTIONS_LIST_SEED: &[u8] = b"sanctions-list";
...
fn process_can_thaw_permissionless(
    ...
    if block_list_pda.data_is_empty() {
        msg!("✅ User {} not in block list - permissionless thaw authorized", token_account_owner.key);
        return Ok(());
    }
    ...
    if record.blocked {
        msg!("❌ User {} is blocked (reason: {:?}) - permissionless thaw denied", 
            token_account_owner.key, record.reason);
        return Err(ProgramError::InvalidAccountData);
    }
    
    msg!("✅ User {} not blocked - permissionless thaw authorized", token_account_owner.key);
    Ok(())
}
...
fn process_can_freeze_permissionless(
    ...
    if block_list_pda.data_is_empty() {
        msg!("❌ User {} not in block list - permissionless freeze denied", token_account_owner.key);
        return Err(ProgramError::InvalidAccountData);
    }
    ...
    if !record.blocked {
        msg!("❌ User {} not blocked - permissionless freeze denied", token_account_owner.key);
        return Err(ProgramError::InvalidAccountData);
    }
    
    msg!("✅ User {} is blocked (reason: {:?}) - permissionless freeze authorized", 
        token_account_owner.key, record.reason);
    Ok(())
}
